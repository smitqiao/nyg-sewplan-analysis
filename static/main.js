// project_nytg/sewing_plan_analysis/static/main.js
        ...
        window.groupBy = groupBy; // <-- ensure export/readers see current groupBy
        ...
        // New: By-Style modal selections (default only 'Style')
        let selectedByStyleOptions = ['Style'];
        ...
        function updateDateInputs() {
            // Find min/max date from allData for SEW_DATE and DELIVERY_DATE
            const sewDates = allData
                ...
                .filter(Boolean);
            const shipDates = allData
                .map(row => row.DELIVERY_DATE || row.delivery_date)
                .filter(Boolean)
                .map(d => {
                    const dateObj = new Date(d);
                    if (!isNaN(dateObj)) return dateObj.toISOString().split('T')[0];
            ...
            if (sewDates.length > 0) {
                minDate = sewDates.reduce((a, b) => a < b ? a : b);
                maxDate = sewDates.reduce((a, b) => a > b ? a : b);
            }
            const dateStart = document.getElementById('dateStart');
            const dateEnd = document.getElementById('dateEnd');
            if (dateStart && dateEnd && minDate && maxDate) {
            ...
            const shipMin = shipDates.length > 0 ? shipDates.reduce((a, b) => a < b ? a : b) : '';
            const shipMax = shipDates.length > 0 ? shipDates.reduce((a, b) => a > b ? a : b) : '';
            const shipStartEl = document.getElementById('shipDateStart');
            const shipEndEl = document.getElementById('shipDateEnd');
            if (shipStartEl && shipEndEl) {
                if (shipMin) {
                    shipStartEl.min = shipMin;
                    shipEndEl.min = shipMin;
                }
                if (shipMax) {
                    shipStartEl.max = shipMax;
                    shipEndEl.max = shipMax;
                }
                // default to full shipment range if exists, otherwise empty
                shipStartEl.value = shipMin || '';
                shipEndEl.value = shipMax || '';
            }
        }
        ...
        function renderDashboard(filteredData) {
            ...
            if (expandLineStyle) {
                // Expanded: group by line + style and optionally product_type (when selectedByStyleOptions includes 'Product type')
                // Use prefixed keys 'S:' and 'P:' inside aggMap[line] to keep both kinds.
                filteredData.forEach(row => {
                    const line = row.PROD_LINE || row.prod_line || 'Unknown';
                    const style = row.STYLE_REF || row.style_ref || 'Unknown';
                    const prodType = row.PRODUCT_TYPE || row.product_type || 'Unknown';
                    ...
                    // Style entry (always present when expanded)
                    const styleKey = 'S:' + style;
                    if (!aggMap[line][styleKey]) aggMap[line][styleKey] = {};
                    if (!aggMap[line][styleKey][groupKey]) aggMap[line][styleKey][groupKey] = { pcsSum: 0, samIeSum: 0, rows: [], groupLabel, subLabel: style, subType: 'style' };
                    const pcs = Number(row.PLAN_PCS || row.plan_pcs) || 0;
                    const sam = Number(row.SAM_IE || row.sam_ie) || 0;
                    aggMap[line][styleKey][groupKey].pcsSum += pcs;
                    aggMap[line][styleKey][groupKey].samIeSum += pcs * sam;
                    aggMap[line][styleKey][groupKey].rows.push(row);

                    // Product type entry (only if selected)
                    if (selectedByStyleOptions.includes('Product type')) {
                        const prodKey = 'P:' + prodType;
                        if (!aggMap[line][prodKey]) aggMap[line][prodKey] = {};
                        if (!aggMap[line][prodKey][groupKey]) aggMap[line][prodKey][groupKey] = { pcsSum: 0, samIeSum: 0, rows: [], groupLabel, subLabel: prodType, subType: 'product' };
                        aggMap[line][prodType][groupKey].pcsSum += pcs;
                        aggMap[line][prodType][groupKey].samIeSum += pcs * sam;
                        aggMap[line][prodType][groupKey].rows.push(row);
                    }
                });
            } else {
    ...
    if (expandLineStyle) {
        ...
        Object.keys(aggMap).sort().forEach(line => {
            ...
            Object.keys(aggMap[line]).sort().forEach(style => {
                // style may be prefixed 'S:' (style) or 'P:' (product type) under the new behavior
                const subLabel = style.startsWith('S:') || style.startsWith('P:') ? style.slice(2) : style;
                html += `<tr><td style="background:${bgColor};">${line} <span style="color:#888;">/</span> <span style="font-weight:bold;background:none;">${subLabel}</span></td>`;
                displayGroups.forEach(groupKey => {
                    let cellValue = '';
                    ...
                    if (styleColorChecked && cellRows.length > 0) {
                        ...
                        if (style.startsWith('S')) {
                            ...
                            if (actualStyleName && styleColorMap[actualStyleName]) {
                                cellStyle = `background:${styleColorMap[actualStyleName]} !important;color:#111 !important;`;
                                cellClass = 'cell-row-style-color';
                                cellValue = actualStyleName;
                            } else {
                                cellValue = '';
                            }
        ...
        let lineIdx = 0;
        const styleToggleEl = document.getElementById('toggleStyleColors');
        Object.keys(aggMap).sort().forEach(line => {
            ...
            rowsToRender.forEach((rowType, idxInGroup) => {
                // add tr with data attributes so click handler can skip non-style rows and read line easily
                html += `<tr data-row-type="${rowType}" data-line="${line}">`;
                if (idxInGroup === 0) {
                    html += `<td rowspan="${rowspan}" style="background:${bg};">${line}</td>`;
                }

                // Render each date column cell
                displayGroups.forEach(groupKey => {
                    let cellValue = '';
                    let cellStyle = '';
                    let cellClass = '';
                    ...
                    const delayColorChecked = document.getElementById('toggleCellColors')?.checked;
                    if (delayColorChecked && selectedDelayStatuses.length > 0) {
                        function getStatusLocal(delivery, sew) {
                            const dDate = new Date(delivery);
                            const sDate = new Date(sew);
                            if (isNaN(dDate) || isNaN(sDate)) return '';
                            const diff = Math.floor((dDate - sDate) / (1000 * 60 * 60 * 24));
                        ...
                        cellRows = cellRows.filter(r => selectedDelayStatuses.includes(getStatusLocal(r.DELIVERY_DATE || r.delivery_date, r.SEW_DATE || r.sew_date)));
                    }

                    // Subprocess/points behavior applies irrespective of rowType
                    if (showSubprocess && selectedSubprocesses.length > 0) {
                        if (usePoints) {
                            const soNoDocSelected = getSelectedValues('soNoDocSelect');
                            if (soNoDocSelected.length === 1) {
                                ...
                                cellRows.forEach(r => {
                                    if ((r.SO_NO_DOC || r.so_no_doc) === soNoDocSelected[0]) {
                                        const vals = sumSubprocess(r, selectedSubprocesses, true, soNoDocSelected[0]);
                                        selectedSubprocesses.forEach(sp => { if (vals[sp] > maxBySub[sp]) maxBySub[sp] = vals[sp]; });
                                    }
                                });
                                let sumMax = 0;
                                selectedSubprocesses.forEach(sp => { sumMax += maxBySub[sp]; });
                            ...
                            let spSum = 0;
                            cellRows.forEach(r => { spSum += sumSubprocess(r, selectedSubprocesses, false); });
                            cellValue = spSum ? formatNumberThousandSep(spSum) : '';
                        }
                    } else {
                        // Non-subprocess rendering depends on rowType
                        if (rowType === 'Style') {
                            const styleColorChecked = document.getElementById('toggleStyleColors')?.checked;
                            if (styleColorChecked && cellRows.length > 0) {
                                const styleSums = {};
                                cellRows.forEach(r => {
                                    const s = r.STYLE_REF || r.style_ref;
                                    const pcs = Number(r.PLAN_PCS || r.plan_pcs) || 0;
                                    if (!s) return;
                                    styleSums[s] = (styleSums[s] || 0) + pcs;
                                });
                                let maxStyle = '';
                                let maxSum = -1;
                                Object.entries(styleSums).forEach(([s, sum]) => { if (sum > maxSum) { maxSum = sum; maxStyle = s; } });
                                ...
                                if (useSamIeCalc) {
                                    ...
                                    let pcsSum = 0;
                                    cellRows.forEach(r => { pcsSum += Number(r.PLAN_PCS || r.plan_pcs) || 0; });
                                    cellValue = pcsSum ? formatNumberThousandSep(pcsSum) : '';
                                }
                            }
                        } else if (rowType === 'Product type') {
                            const ptSums = {};
                            cellRows.forEach(r => {
                                const pt = r.PRODUCT_TYPE || r.product_type || 'Unknown';
                                const pcs = Number(r.PLAN_PCS || r.plan_pcs) || 0;
                                ptSums[pt] = (ptSums[pt] || 0) + pcs;
                            });
                            let maxPt = '';
                            let maxPtSum = -1;
                    ...
                    if (delayColorChecked && cellRows.length > 0 && !cellClass) {
                        ...
                        if (!hasSelectedStatus) {
                            ...
                            else if (foundStatus === 'Too early') { cellStyle = 'background:#e0e7ff !important;color:#111 !important;'; cellClass = 'cell-row-too-early'; }
                            else if (foundStatus === 'Ok') { cellStyle = 'background:#dcfce7 !important;color:#111 !important;'; cellClass = 'cell-row-ok'; }
                        }
                    }

                    // Mark product/efficiency cell rows visually and make them non-clickable by adding 'no-click' class on TR (click handler will skip)
                    if (rowType !== 'Style') {
                        // add an indicator class on the row via cellClass (so styling persists) but click handler will use data-row-type to skip
                    }

                    if (!cellValue) cellStyle = cellStyle || 'background:#CECECE !important;color:#111 !important;';
                    html += `<td class="date-col${cellClass ? ' ' + cellClass : ''}" data-full-date="${groupKey}"${cellStyle ? ` style="${cellStyle}"` : ''}>${cellValue}</td>`;
                });
                html += '</tr>';
            });
    ...
    // Choices.js instances
            ...
            function getCurrentFilters() {
                return {
                    ...
                    shipDateStart: document.getElementById('shipDateStart') ? document.getElementById('shipDateStart').value : '',
                    shipDateEnd: document.getElementById('shipDateEnd') ? document.getElementById('shipDateEnd').value : ''
                };
            }
            ...
            function getFilteredForDropdown(dropdownId) {
                ...
                // ensure shipDate fields exist for tempFilters
                tempFilters.shipDateStart = tempFilters.shipDateStart || '';
                tempFilters.shipDateEnd = tempFilters.shipDateEnd || '';
                return allData.filter(row => {
                    ...
                    if (dateRaw) {
                        const dateObj = new Date(dateRaw);
                        if (!isNaN(dateObj)) date = dateObj.toISOString().split('T')[0];
                    }
                    if (shipRaw) {
                        const sObj = new Date(shipRaw);
                        if (!isNaN(sObj)) shipDate = sObj.toISOString().split('T')[0];
                    }
                    return (
                        ...
                        (!tempFilters.shipDateStart || shipDate >= tempFilters.shipDateStart) &&
                        (!tempFilters.shipDateEnd || shipDate <= tempFilters.shipDateEnd)
                    );
                });
            }
            ...
            function resetFilters() {
                ...
                // reset shipment dates to their defaults (set in updateDateInputs)
                const shipStartEl = document.getElementById('shipDateStart');
                const shipEndEl = document.getElementById('shipDateEnd');
                if (shipStartEl && shipEndEl) {
                    // set to min/max of shipment range if present
                    shipStartEl.value = shipStartEl.min || '';
                    shipEndEl.value = shipEndEl.max || '';
                }
                updateAllDropdownsExceptFactory();
                filterAndRender();
                                             }
            ...
            fetch('/api/plan_data')
                ...
                .then(data => {
                    ...
                    // Add change listeners for shipment date inputs
                    const shipStartEl = document.getElementById('shipDateStart');
                    const shipEndEl = document.getElementById('shipDateEnd');
                    if (shipStartEl) shipStartEl.addEventListener('change', function() { showLoadingSpinnerOverlay(); setTimeout(filterAndRender,0); });
                    if (shipEndEl) shipEndEl.addEventListener('change', function() { showLoadingSpinnerOverlay(); setTimeout(filterAndRender,0); });
                    ...
                    document.getElementById('groupBySelect').addEventListener('change', function() {
                        groupBy = this.value;
                        // Keep global in sync so export and other scripts read the correct grouping
        ...
        filteredData.forEach(row => {
            ...
            if (showDelay) {
                ...
                if (!isNaN(dDate) && !isNaN(sDate)) {
                    ...
                    else if (diff <= 5 && diff >= 0) status = 'CanBeDelay';
    ...
    // --- By Style Modal (open when clicking the "By Style" label) ---
    const byStyleModalId = 'byStyleModal';
    if (!document.getElementById(byStyleModalId)) {
        const modal = document.createElement('div');
        modal.id = byStyleModalId;
        modal.className = 'modal-overlay';
        modal.style.zIndex = '10001';
        modal.innerHTML = `
            <div class="modal-content" style="min-width:320px;max-width:96vw;">
                ...
                <div style="margin-top:16px;display:flex;justify-content:flex-end;gap:12px;">
                    ...
                    <button type="button" id="byStyleModalCancelBtn" style="background:#eee;color:#333;border:none;border-radius:5px;padding:6px 18px;">Cancel</button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    ...
    // Connect label click for style toggle to open modal (and apply hover like delay)
    const styleLabel = document.querySelector('label[for="toggleStyleColors"]');
    if (styleLabel) {
        styleLabel.style.cursor = 'pointer';
        styleLabel.style.transition = 'background 0.15s, color 0.15s';
        styleLabel.addEventListener('mouseenter', function() {
            styleLabel.style.background = '#e0f2fe';
            styleLabel.style.color = '#0369a1';
        });
        styleLabel.addEventListener('mouseleave', function() {
            styleLabel.style.background = '';
            styleLabel.style.color = '';
        });
        styleLabel.addEventListener('click', function(e) {
            e.preventDefault();
    ...
    document.getElementById('byStyleModalCancelBtn').onclick = function() {
        document.getElementById(byStyleModalId).style.display = 'none';
    };
    document.getElementById(byStyleModalId).addEventListener('click', function(e) {
        if (e.target === this) this.style.display = 'none';
    });

    // OK/Cancel logic for Delay Status modal
    document.getElementById('delayModalOkBtn').onclick = function() {
        const form = document.getElementById('delayStatusForm');
        selectedDelayStatuses = Array.from(form.elements)
            .filter(el => el.type === 'checkbox' && el.checked)
            .map(el => el.value);
        // Default to all statuses if nothing selected
        if (selectedDelayStatuses.length === 0) {
            selectedDelayStatuses = ['Delay', 'Can be delay', 'Too early', 'Ok'];
        }
